...
export async function addAnimeToFavorites(req: Request, res: Response,next:NextFunction) {

    const animeId = (req.body as { animeId: string }).animeId;
    try {
        await primsa.favoriteAnime.create({
            data: {
                userId: req.user_id,
                animeId
            }
...
export async function removeAnimeFromFavorites(req: Request, res: Response,next:NextFunction) {
    const animeSequenceId = (req.body as { animeSequenceId: string }).animeSequenceId;

    try {
     await primsa.favoriteAnime.delete({
            where: {
                id: animeSequenceId
            }
        });
...
export async function fetchUserFavorites(req: Request, res: Response,next:NextFunction) {

    try {
        const data = await primsa.favoriteAnime.findMany({
            where: {
                userId: req.user_id
            },
            orderBy: {
