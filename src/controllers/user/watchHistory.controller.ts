...
export async function addAnimeEpisodeToWatchHistory(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export async function removeAnimeEpisodeFromWatchHistory(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  const animeSequenceEpisodeId = (
    req.body as { animeSequenceEpisodeId: string }
  ).animeSequenceEpisodeId;

  try {
    const response = await prisma.watchHistory.delete({
      where: {
...
export async function fetchUserWatchHistory(
  ...
  try {
    const data = await prisma.watchHistory.findMany({
      where: {
        userId: req.user_id,
      },
      orderBy: {
...
export async function deleteUserWatchHistory(
  req: Request,
  res: Response,
  next: NextFunction,
) {
